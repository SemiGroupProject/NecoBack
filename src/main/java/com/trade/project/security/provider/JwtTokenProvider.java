package com.trade.project.security.provider;
...
public class JwtTokenProvider { // JWT 토큰 생성 및 검증 모듈
    ...
    private long tokenValidMilliSecond = 1000 * 60 * 60; // 1시간만 토큰 유효

    private final UserDetailsService customUserDetailService;

    //TODO: 왜 protected로 둬야할까?
    @PostConstruct
    protected void init() {
    ...
    public Authentication getAuthentication(String token) {
        UserDetails userDetails = customUserDetailService.loadUserByUsername(this.getUserId(token.substring("Bearer ".length())));
